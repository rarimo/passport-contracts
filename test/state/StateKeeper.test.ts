...
describe("StateKeeper", () => {
  ...
  describe("$Contract management", () => {
    ...
    describe("#removeCertificateMock", () => {
      const certificateKey = ethers.keccak256(ethers.toUtf8Bytes("legacy-rsa-certificate"));

      beforeEach(async () => {
        await addRegistrations(["Certificate Registration"], [ADDRESS1.address]);
        await stateKeeper.addCertificate(certificateKey, (await time.latest()) + 365 * 24 * 60 * 60);
      });

      it("should let an owner remove an unexpired certificate with the canonical effects", async () => {
        const rootBefore = await certificatesSmt.getRoot();

        await expect(stateKeeper.removeCertificateMock(certificateKey))
          .to.emit(stateKeeper, "CertificateRemoved")
          .withArgs(certificateKey);

        expect((await stateKeeper.getCertificateInfo(certificateKey)).expirationTimestamp).to.equal(0);
        expect(await certificatesSmt.getRoot()).not.to.equal(rootBefore);
      });

      it("should reject a non-owner", async () => {
        await expect(stateKeeper.connect(ADDRESS2).removeCertificateMock(certificateKey))
          .to.be.revertedWithCustomError(stateKeeper, "UnauthorizedAccount")
          .withArgs(ADDRESS2.address);
      });

      it("should reject a missing certificate instead of silently masking inconsistent state", async () => {
        const missingKey = ethers.keccak256(ethers.toUtf8Bytes("missing-certificate"));

        await expect(stateKeeper.removeCertificateMock(missingKey)).to.be.revertedWith(
    ...
    describe("#migrateCertificateMockBatch", () => {
      const certificate = {
        dataType: ethers.ZeroHash,
        signedAttributes: "0x",
        keyOffset: 0,
        expirationOffset: 0,
      };

      it("should expose the bounded transaction size", async () => {
        expect(await stateKeeper.MAX_CERTIFICATE_MIGRATION_BATCH()).to.equal(3);
      });

      it("should reject an empty batch", async () => {
        await expect(stateKeeper.migrateCertificateMockBatch([], [], ADDRESS1.address)).to.be.revertedWith(
      ...
      it("should reject mismatched batch arrays", async () => {
        await expect(
          stateKeeper.migrateCertificateMockBatch([ethers.ZeroHash], [], ADDRESS1.address),
        ).to.be.revertedWith("StateKeeper: certificate batch length mismatch");
      });

      it("should reject a batch larger than the bounded transaction size", async () => {
        await expect(
          stateKeeper.migrateCertificateMockBatch(
            Array(4).fill(ethers.ZeroHash),
            Array(4).fill(certificate),
            ADDRESS1.address,
          ),
        ).to.be.revertedWith("StateKeeper: invalid certificate batch size");
      });

      it("should reject a non-owner before processing the batch", async () => {
        await expect(
          stateKeeper.connect(ADDRESS2).migrateCertificateMockBatch([ethers.ZeroHash], [certificate], ADDRESS1.address),
        )
          .to.be.revertedWithCustomError(stateKeeper, "UnauthorizedAccount")
          .withArgs(ADDRESS2.address);
      });

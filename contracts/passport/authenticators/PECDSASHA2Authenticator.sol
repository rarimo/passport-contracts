...
contract PECDSASHA2Authenticator {
    ...
    function _multiplyScalarPartialOpt(
        bytes memory x0,
        bytes memory y0,
        bytes memory scalar
    ) public view returns (BigIntOpt memory x1, BigIntOpt memory y1, BigIntOpt memory z1) {
        BigIntOpt memory x0_ = x0.initOpt(false);
        BigIntOpt memory y0_ = y0.initOpt(false);
        BigIntOpt memory scalar_ = scalar.initOpt(false);
        ...
        BigIntOpt memory base2X = x0_;
        BigIntOpt memory base2Y = y0_;
        BigIntOpt memory base2Z = oneOpt;

        y1 = y0_.val.initOpt(false);
        z1 = oneOpt;
        x1 = x0_.val.initOpt(false);

        scalar_.mod(twoOpt, temps.temp);
        if (temps.temp.eq(oneOpt)) {
            x1 = zeroOpt;
            y1 = zeroOpt;
        }
        ...
        for (uint i = 0; i < 5; i++) {
            ...
            scalar_.mod(twoOpt, temps.temp);
            if (temps.temp.eq(oneOpt)) {
                _addProjOpt(base2X, base2Y, base2Z, x1, y1, z1, temps.temp);
            }
    ...
    function _twiceProjOpt(
        BigIntOpt memory x0,
        BigIntOpt memory y0,
        BigIntOpt memory z0,
        BigIntOpt memory tempU,
        BigIntOpt memory tempV,
        BigIntOpt memory tempT,
        BigIntOpt memory tempW,
        ...
        BigIntsOpt.modmul(y0, z0, pOpt, tempU);
        BigIntsOpt.modmul(tempU, twoOpt, pOpt, tempU);

        BigIntsOpt.modmul(tempU, x0, pOpt, tempV);
        BigIntsOpt.modmul(tempV, y0, pOpt, tempV);
        BigIntsOpt.modmul(tempV, twoOpt, pOpt, tempV);

        BigIntsOpt.modmul(x0, x0, pOpt, x0);
        BigIntsOpt.modmul(x0, 3.initOpt(false), pOpt, tempT);

        BigIntsOpt.modmul(z0, z0, pOpt, z0);
        BigIntsOpt.modmul(z0, aOpt, pOpt, z0);

        // //t = (t + z0) % p;
        tempT.add(z0, tempT);
        tempT.mod(pOpt, tempT);

        BigIntsOpt.modmul(tempT, tempT, pOpt, tempW);
        BigIntsOpt.modmul(twoOpt, tempV, pOpt, x0);

        // //w = (w + (p - x0)) % p;
        pOpt.sub(x0, temp);
        tempW.add(temp, tempW);
        tempW.mod(pOpt, tempW);
        ...
        BigIntsOpt.modmul(tempT, x0, pOpt, x0);
        BigIntsOpt.modmul(y0, tempU, pOpt, y0);
        BigIntsOpt.modmul(y0, y0, pOpt, y0);
        BigIntsOpt.modmul(twoOpt, y0, pOpt, y0);
        ...
        BigIntsOpt.modmul(tempU, tempW, pOpt, x0);

        BigIntsOpt.modmul(tempU, tempU, pOpt, z0);
        BigIntsOpt.modmul(z0, tempU, pOpt, z0);
    }
    ...
    function _addProjOpt(
        ...
        BigIntsOpt.modmul(y0, z1, pOpt, t0);
        BigIntsOpt.modmul(y1, z0, pOpt, t1);

        BigIntsOpt.modmul(x0, z1, pOpt, u0);
        BigIntsOpt.modmul(x1, z0, pOpt, u1);
        ...
        BigIntsOpt.modmul(z0, z1, pOpt, temp);

        _addProj2Opt(temp, u0, u1, t1, t0, x1, y1, z1);
    }
    ...
    function _addProj2Opt(
        ...
        BigIntsOpt.modmul(u, u, pOpt, u2);

        BigIntsOpt.modmul(t, t, pOpt, w);
        BigIntsOpt.modmul(w, v, pOpt, w);

        u1.add(u0, u1);
        u1.mod(pOpt, u1);
        BigIntsOpt.modmul(u1, u2, pOpt, u1);

        pOpt.sub(u1, x0);
        w.add(x0, w);
        w.mod(pOpt, w);

        BigIntsOpt.modmul(u, w, pOpt, x0);

        BigIntsOpt.modmul(u2, u, pOpt, u3);

        BigIntsOpt.modmul(u0, u2, pOpt, u0);

        pOpt.sub(w, w);
        u0.add(w, u0);
        u0.mod(pOpt, u0);

        BigIntsOpt.modmul(t, u0, pOpt, t);
        BigIntsOpt.modmul(t0, u3, pOpt, t0);

        pOpt.sub(t0, t0);
        t.add(t0, t);
        t.mod(pOpt, y0);

        BigIntsOpt.modmul(u3, v, pOpt, z0);
    }
